...
- **FR-14** — Handle event kinds `output`, `input`, `status`, `exit`, and `kill` (`kill` is live-verified Aug 3 but undocumented upstream; it is a known silent kind). Unknown kinds MUST be ignored gracefully and increment `diagnostics.unknownEvents`.
...
1. **Resolved Aug 7, 2026 — official brief and event repository verified.** Deadline (before Sat Aug 8, 00:00 EDT), freeze tag (`august-hackathon-2026-final`), submission URL, MIT/DCO rules, ≤5-minute demo rule, participant templates, and issue form are confirmed. `LICENSE` and `HACKATHON.md` are reconciled to the official templates. The README contains the guide's 14 required sections plus two permitted extras; scoring is 100 core / 60 to qualify / 25 bonus with seven unweighted criteria.
